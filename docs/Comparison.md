# wolfNanoTLS vs MbedTLS

An apples-to-apples crypto benchmark, modeled on the wolfSSL vs MbedTLS
technical benchmark. wolfNanoTLS links wolfCrypt's assembly through the `wc_*`
seam, so this is wolfCrypt's speed delivered from wolfNanoTLS's slim, no-configure
...
- **wolfNanoTLS:** `WOLFNANOTLS_ASM=intel` (AES-NI, AVX1/2, single-precision x86_64
...
| Operation | wolfNanoTLS | MbedTLS | Advantage |
|---|--:|--:|--:|
...
The same speed-tuned wolfNanoTLS build also ships algorithms MbedTLS lacks
entirely. Measured here (wolfNanoTLS `intel`, ops/s; MbedTLS: none):

| Operation | wolfNanoTLS | MbedTLS |
|---|--:|:--:|
...
Built the same way on the same hardware, wolfNanoTLS (wolfCrypt's asm via the slim
