...
/* Compile-time X.509 feature toggles (algorithm recognition tracks the compiled
 * crypto via HAVE_ECC384 / WOLFSSL_SHA384 / WOLFSSL_SHA512 / HAVE_ED25519 / NO_RSA):
 *   WOLFNANO_X509_HOSTNAME   - parse SAN dNSName + CN for hostname matching (opt-in)
 *   WOLFNANO_NO_X509_CN      - drop commonName parse, SAN-only (needs HOSTNAME)
 *   WOLFNANO_NO_X509_TIME    - drop validity Time parse + wn_X509_TimeValid
 *   WOLFNANO_NO_X509_GENTIME - drop GeneralizedTime, accept UTCTime only
 *   WOLFNANO_NO_X509_PATHLEN - drop optional basicConstraints pathLen parse
 */
