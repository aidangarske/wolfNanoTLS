...
| Approved-mode ECDHE P-256 (`WOLFNANOTLS_FIPS`) | done, interop verified |
| `WOLFNANOTLS_ASM=<arch>` speedup switch | done |
...
| Arch (`WOLFNANOTLS_ASM=`) | Build | Run / measure |
|---|---|---|
...
1. **Cortex-M33 (STM32H563) — the priority.** Build `WOLFNANOTLS_ASM=thumb2`,
...
2. **mbedTLS head-to-head footprint** on a small Cortex-M part: gc'd,
   ...
   it with the on-device speed run. wolfNanoTLS's own size method is in
...
- `WOLFNANOTLS_ASM=<arch>` (Makefile) → sets `-DWOLFNANOTLS_TARGET_<X>` (macro bundle
...
- Feature flags stay per-target `-DWOLFNANOTLS_*` (see [Macros.md](Macros.md)); no
  `./configure`.
