...
static int wn_CertTimeValid(const DecodedCert* cert, time_t now)
{
    ...
    if ((cert->beforeDate == NULL) || (cert->afterDate == NULL)) {
        ret = WOLFNANO_E_BAD_CERT;          /* a server cert must carry dates */
    }
    if (ret == WOLFNANO_SUCCESS) {
        if (wc_GetDateInfo(cert->beforeDate, cert->beforeDateLen, &d, &fmt,
            ...
            ret = WOLFNANO_E_BAD_CERT;       /* not yet valid */
        }
    }
    if (ret == WOLFNANO_SUCCESS) {
        if (wc_GetDateInfo(cert->afterDate, cert->afterDateLen, &d, &fmt,
            ...
            ret = WOLFNANO_E_BAD_CERT;       /* expired */
        }
    }
                          ...
                          )
{
    ...
    word32 issuerLen;
    byte anchorPub[640];       /* anchor public key (fits RSA-4096 and smaller) */
    word32 anchorPubSz = 0;
    int i;
    int notAnchor;
    int matched;
    int ret = WOLFNANO_SUCCESS;
    ...
    /* capture the anchor key so the walk can stop at a re-sent anchor */
    if (ret == WOLFNANO_SUCCESS) {
        wc_InitDecodedCert(&leaf, anchor, anchorLen, NULL);
        if ((wc_ParseCert(&leaf, CERT_TYPE, NO_VERIFY, NULL) == 0) &&
            (leaf.pubKeySize <= (word32)sizeof(anchorPub))) {
            XMEMCPY(anchorPub, leaf.publicKey, leaf.pubKeySize);
            anchorPubSz = leaf.pubKeySize;
        }
        wc_FreeDecodedCert(&leaf);
    }

    for (i = 0; (i < n) && (ret == WOLFNANO_SUCCESS); i++) {
        /* server may re-send the (cross-signed) root; stop at the anchor key */
        matched = 0;
        wc_InitDecodedCert(&leaf, certs[i], certLens[i], NULL);
        if (wc_ParseCert(&leaf, CERT_TYPE, NO_VERIFY, NULL) == 0) {
            matched = (anchorPubSz > 0) && (leaf.pubKeySize == anchorPubSz) &&
                      (ConstantCompare(leaf.publicKey, anchorPub,
                                       (int)anchorPubSz) == 0);
        }
        wc_FreeDecodedCert(&leaf);
        if (matched) {
    ...
    if (ret == WOLFNANO_SUCCESS) {
        ...
        if (wc_ParseCert(&leaf, CERT_TYPE, NO_VERIFY, NULL) != 0) {
            ...
            if ((ret == WOLFNANO_SUCCESS) && (leaf.extKeyUsageSet != 0) &&
                ((leaf.extKeyUsage & KEYUSE_DIGITAL_SIG) == 0)) {
                ret = WOLFNANO_E_BAD_CERT;   /* leaf must allow digitalSignature */
            }
            if ((ret == WOLFNANO_SUCCESS) && (leaf.extExtKeyUsageSet != 0) &&
    ...
    const byte* issuerDer;
    word32 issuerLen;
    const byte* anchorSpki = NULL;
    word32 anchorSpkiLen = 0;
    int i;
    int notAnchor;
    int ret = WOLFNANO_SUCCESS;
    ...
    /* capture the anchor key so the walk can stop at a re-sent anchor */
    if ((ret == WOLFNANO_SUCCESS) &&
        (wn_X509_Parse(&issuer, anchor, anchorLen) == WOLFNANO_SUCCESS)) {
        anchorSpki = issuer.spki;
        anchorSpkiLen = issuer.spkiLen;
    }
    ...
    for (i = 0; (i < n) && (ret == WOLFNANO_SUCCESS); i++) {
        ...
        /* server may re-send the (cross-signed) root; stop at the anchor key */
        if ((anchorSpki != NULL) && (child.spkiLen == anchorSpkiLen) &&
            (ConstantCompare(child.spki, anchorSpki, (int)anchorSpkiLen) == 0)) {
            break;
        }
        if ((i + 1) < n) {
            issuerDer = certs[i + 1];
        ...
        notAnchor = ((i + 1) < n) &&
                    ((issuerLen != anchorLen) ||
                     (ConstantCompare(issuerDer, anchor, (int)anchorLen) != 0));
        if (wn_X509_Parse(&issuer, issuerDer, issuerLen) != WOLFNANO_SUCCESS) {
            ret = WOLFNANO_E_BAD_CERT;
        }
        if ((ret == WOLFNANO_SUCCESS) &&
