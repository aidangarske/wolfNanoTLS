#!/bin/sh
# Whole TLS 1.3 SERVER footprint: wolfNanoTLS vs MbedTLS vs wolfSSL, all built from
# source for Cortex-M33 (Thumb2, -Os, --gc-sections), every library configured to
# the same minimal scope (ECDHE P-256 + X25519, AES-128-GCM, SHA-2; cert build
# adds X.509 + ECDSA sign). Reports .text bytes. Static, no run. Mirrors
# footprint-clients.sh for the WOLFNANO_SERVER adder.
set -u
...
# server shell: shared handshake + ClientHello decoder + ServerHello encoder +
# accept state machine (no wn_connect; cert adds wn_servercert).
...
# cert / X.509 P-256 (ECDSA server-cert sign): ecc + asn + wn_servercert
$CC -I"$(cfg cert)" -DWOLFNANO_X509 $WN_BASE \
   $WN_SUP $WC/ecc.c $WC/asn.c src/wn_servercert.c $WN_SHELL \
   bench/min/wn_server.c $LINK -o "$OUT/wn_cert.elf" 2>/dev/null
...
WS_CF="$OPT $ARCH -DWOLFSSL_USER_SETTINGS -DWOLFNANO_TARGET_PORTABLE_C -Ibench/min/ws_srv -Iwolfssl"
$CC $WS_CF $WC/wc_port.c $WC/memory.c $WC/error.c $WC/hash.c $WC/random.c $WC/wolfmath.c \
   ...
   bench/min/ws/ws_cert_server.c $LINK -o "$OUT/ws_cert.elf" 2>/dev/null
