/* mbedTLS minimal PSK (+ECDHE) TLS 1.3 client+server device, no X.509: size-only harness. */

#include "mbedtls4_config_psk_cs.h"
...
int main(void)
{
    ...
    ret += mbedtls_ssl_config_defaults(&sconf, MBEDTLS_SSL_IS_SERVER,
               MBEDTLS_SSL_TRANSPORT_STREAM, MBEDTLS_SSL_PRESET_DEFAULT);
    ret += mbedtls_ssl_conf_psk(&cconf, psk, sizeof(psk), id, sizeof(id) - 1);
    ret += mbedtls_ssl_conf_psk(&sconf, psk, sizeof(psk), id, sizeof(id) - 1);
