...
wolfNanoTLS's CI runs on GitHub Actions. Because wolfNanoTLS compiles the **pinned
...
submodule at a ref and running wolfNanoTLS's Makefile.
...
- `interop.yml` - `make interop`: wolfNanoTLS client vs OpenSSL, wolfSSL, and mbedTLS servers (PSK + cert).

### Standards
- `house-style.yml` - no `//`, no em/en-dash, no tabs, no trailing whitespace.
- `empty-brace-scan.yml` - no bare scope blocks.
- `no-alloc-scan.yml` - no raw libc allocation in the product source (static).
- `no-malloc-runtime.yml` - runtime proof: runs the crypto path and the full PSK
  handshake under a `--wrap` allocation probe, asserting zero allocations
  (Linux/GNU ld only).
