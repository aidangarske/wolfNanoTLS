...
int main(void)
{
    ...
    /* basicConstraints value: OCTET STRING { SEQUENCE { BOOLEAN cA=TRUE } } */
    static const byte bc_true[] = {0x04, 0x05, 0x30, 0x03, 0x01, 0x01, 0xFF};
    ...
    XMEMCPY(buf, ca, caLen);
    pos = find_pat(buf, (long)caLen, bc_true, (long)sizeof(bc_true), 0);
    check(pos >= 0, "located basicConstraints cA BOOLEAN");
    if (pos >= 0) {
        buf[pos + 6] = 0x00;            /* cA TRUE (0xFF) -> explicit FALSE */
        check(wn_X509_Parse(&wn, buf, caLen) == WOLFNANO_E_X509_DECODE,
              "basicConstraints explicit cA=FALSE rejected");
    }
