...
static const byte WN_OID_P521[]        = {0x2B,0x81,0x04,0x00,0x23};
...
/* Byte-equality for public data (OIDs, algorithm ids, DigestInfo). These are
 * not secrets, so a plain compare is correct; ConstantCompare is reserved for
 * MAC/secret comparisons on the handshake path. */
...
static int wn_CurveFromOid(const byte* p, int len)
{
    ...
    else if (wn_OidEq(p, len, WN_OID_P521, sizeof(WN_OID_P521))) {
        curve = WN_X509_CURVE_P521;
    }
        ...
        while ((idx < rdnEnd) && (*cn == NULL)) {
            ...
            if (wn_OidEq(der + idx, oidLen, WN_OID_CN, sizeof(WN_OID_CN))) {
                ...
                if (idx < atvEnd) {
                     ...
                     * so a non-string value cannot become a hostname */
                    if (((valTag == 0x0C) || (valTag == 0x13) ||
                         (valTag == 0x14) || (valTag == 0x16)) &&
                          ...
                          wn_X509Cert* cert, byte crit)
{
    ...
    rc = wn_Asn_GetHeader(der, WN_ASN_BIT_STRING, &idx, &kuLen, end);
    if ((rc >= 0) && ((idx + (word32)kuLen) != end)) {
        rc = WOLFNANO_E_X509_DECODE;        /* BIT STRING must fill the extnValue */
    }
...
int wn_X509_Parse(wn_X509Cert* cert, const byte* der, word32 derLen)
{
    ...
    if (rc >= 0) {
         ...
         * and PKCS#1). */
        if ((cert->sigAlg == WN_X509_SIG_ECDSA_SHA256) ||
            (cert->sigAlg == WN_X509_SIG_ECDSA_SHA384) ||
            (cert->sigAlg == WN_X509_SIG_ED25519)) {
            if ((oidIdx + (word32)oidLen) != (idx + (word32)len)) {
                rc = WOLFNANO_E_X509_DECODE;
            }
        }
        else if ((cert->sigAlg == WN_X509_SIG_RSA_SHA256) ||
                 (cert->sigAlg == WN_X509_SIG_RSA_SHA384) ||
                 (cert->sigAlg == WN_X509_SIG_RSA_SHA512)) {
...
static int wn_CurveId(int curve)
{
    ...
    switch (curve) {
        ...
        case WN_X509_CURVE_P521: id = ECC_SECP521R1; break;
        default: break;
    }
    return id;
}
...
static int wn_SigAlgMatchesKey(int keyAlg, int sigAlg)
{
    ...
    switch (keyAlg) {
        ...
        case WN_X509_KEY_RSA:
            ok = (sigAlg == WN_X509_SIG_RSA_SHA256) ||
...
static int wn_CertTimeEpoch(const byte* tlv, word16 tlvLen, long long* epoch)
{
    const byte* p = NULL;
    int         ret = WOLFNANO_SUCCESS;
    int         y = 0, c = 0, mo = 0, d = 0, h = 0, mi = 0, s = 0;
    int         contentLen;
