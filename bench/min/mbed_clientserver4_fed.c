/* mbedTLS 4.x TLS 1.3 cert client+server DEVICE, size-only. Client half sends
 * first (opaque I/O); server half FED the captured ClientHello + real embedded
 * cert, so both roles link under -flto. 4.x delta: PSA-only RNG, 5-arg pk parse. */
