#!/bin/sh
# mbedTLS 4.1.0 TLS 1.3 SERVER footprint (Cortex-M33, -Os, gc-sections): PSK+ECDHE
# X25519, PSK+ECDHE P-256, and X.509 cert (ECDSA P-256). Server-side of the
# tf-psa-crypto build (excludes ssl_tls13_client). Mirrors footprint-mbedtls4*.sh.
set -u
...
EX_COMMON=" net_sockets timing ssl_tls12_client ssl_tls12_server ssl_tls13_client \
...
X=$(build mbedtls4_config_psk_srv.h mbedtls4_crypto_config_psk_x25519.h mbed_psk_server4.c psk_x25519)
P=$(build mbedtls4_config_psk_srv.h mbedtls4_crypto_config_psk_p256.h  mbed_psk_server4.c psk_p256)
C=$(build mbedtls4_config_srv.h  mbedtls4_crypto_config.h           mbed_server4.c     cert)

printf '\nmbedTLS 4.1.0 TLS 1.3 SERVER .text (Cortex-M33, -Os, gc-sections):\n'
printf '  %-24s %s\n' "PSK + ECDHE, X25519" "${X:--}"
printf '  %-24s %s\n' "PSK + ECDHE, P-256"  "${P:--}"
printf '  %-24s %s\n' "cert / X.509, P-256" "${C:--}"
