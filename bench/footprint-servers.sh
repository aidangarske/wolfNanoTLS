#!/bin/sh
# Whole TLS 1.3 SERVER footprint: wolfNanoTLS vs mbedTLS 3.6.0 vs wolfSSL, built
...
WN_SHELL="src/wn_msg.c src/wn_keyschedule.c src/wn_transcript.c src/wn_record.c src/wn_keyshare.c src/wn_serverhello.c src/wn_clienthello.c src/wn_handshake.c src/wn_accept.c"

$CC -I"$(cfg minimal)" $WN_BASE $WN_SUP_PSK $WN_SHELL bench/min/wn_psk_server.c $LINK -o "$OUT/wn_psk.elf" 2>/dev/null
$CC -I"$(cfg psk_p256)" $WN_BASE $WN_SUP_P256 $WN_SHELL bench/min/wn_psk_server.c $LINK -o "$OUT/wn_psk_p256.elf" 2>/dev/null
...
$CC -I"$(cfg cert)" -DWOLFNANO_X509 -DWOLFNANO_X509_LITE $WN_BASE $WN_SUP $WC/ecc.c $WC/asn.c src/wn_x509.c src/wn_servercert.c $WN_SHELL bench/min/wn_server.c $LINK -o "$OUT/wn_cert.elf" 2>/dev/null
# ML-DSA-44 server cert (scheme 0x0904); no -flto (wc_mldsa.c + gc-sections LTO bug on ArmGNU 14.2)
$CC -I"$(cfg cert_mldsa)" -DWOLFNANO_X509 -DWOLFNANO_X509_LITE -DWN_FP_SCHEME=0x0904 $WN_BASE_NOLTO $WN_SUP_MLDSA src/wn_x509.c src/wn_servercert.c $WN_SHELL bench/min/wn_server.c $GCLINK -o "$OUT/wn_cert_mldsa.elf" 2>/dev/null
# X25519MLKEM768 hybrid KEX PSK server
$CC -I"$(cfg pqc)" $WN_BASE $WN_SUP_PQC src/wn_hybrid.c $WN_SHELL bench/min/wn_psk_server.c $LINK -o "$OUT/wn_pqc.elf" 2>/dev/null
...
ws_build bench/min/ws_srv_psk       "$WC/ecc.c $WC/asn.c"          bench/min/ws/ws_psk_server.c    "$OUT/ws_psk.elf"
ws_build bench/min/ws_srv_psk_p256  "$WC/ecc.c $WC/asn.c"          bench/min/ws/ws_psk_server.c    "$OUT/ws_psk_p256.elf"
ws_build bench/min/ws_srv           "$WC/ecc.c $WC/asn.c $WC/rsa.c $WC/ed25519.c $WC/ge_operations.c" bench/min/ws/ws_cert_server.c "$OUT/ws_cert.elf"
ws_build bench/min/ws_srv_mldsa     "$WC/ecc.c $WC/asn.c $WC/sha3.c $WC/wc_mldsa.c $WC/rsa.c $WC/ed25519.c $WC/ge_operations.c" bench/min/ws/ws_cert_server.c "$OUT/ws_mldsa.elf"
ws_build bench/min/ws_srv_pqc       "$WC/ecc.c $WC/asn.c $WC/sha3.c $WC/wc_mlkem.c $WC/wc_mlkem_poly.c" bench/min/ws/ws_psk_server.c "$OUT/ws_pqc.elf"
