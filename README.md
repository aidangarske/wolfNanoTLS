...
<img src="docs/images/wolfnano-logo.png" width="290" alt="wolfNano logo" />
...
`arm-none-eabi-gcc -Os -flto --gc-sections` + nano specs, with wolfNano and
...
client at ~80 KB, so against a typical mbedTLS the gap is larger; wolfNano quotes
the hard-minimized number because it is the fair one.

The default curve is **X25519** (smallest); set `WOLFNANO_HAVE_ECDHE_P256`
(or `WOLFNANO_FIPS`) to negotiate **P-256** for FIPS / broad interop. Both are
interop-verified against OpenSSL and wolfSSL; see
[curve selection](https://github.com/aidangarske/wolfNano/wiki/Macros).
...
(mbedTLS) and `bench/min/wnc/user_settings.h` (wolfNano). Both harness clients
